...
public final class BossMessage {
    ...
    private final static Map<Integer, BarUtil> bar_objects = new LinkedHashMap<>();
     ...
     */
    private void displayBar(Collection<ProxiedPlayer> players) {
        bars++;
        if (cancelled)
            cancelled = false;
        ...
        BarUtil bar = BarUtil.builder()
                .color(color)
                .style(type)
                .player(players.toArray(new ProxiedPlayer[0]))
                .health(1.0F)
                .title(TextComponent.fromLegacyText(StringUtils.toColor(message))).build();
        bar.setVisible(true);
