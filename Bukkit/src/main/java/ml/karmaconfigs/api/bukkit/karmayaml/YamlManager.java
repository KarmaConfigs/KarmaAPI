...
public final class YamlManager {
     ...
     */
    public YamlManager(final JavaPlugin plugin, final String fileName, final String... directory) {
        main = plugin;
        ...
        managed = YamlConfiguration.loadConfiguration(file);
    }
        ...
        try {
            ...
            if (internal_name != null && !internal_name.replaceAll("\\s", "").isEmpty()) {
                ...
                if (reloader.reloadAndCopy())
                    managed.loadFromString(reloader.getYamlString());
            }
        } catch (Throwable ex) {
